...
class VideoLogic {
  ...
  void init() async{
    ...
    context.read<VideoCallBloc>().add(UserInfoChanged(data["to_avatar"] ?? "", data["to_name"] ?? ""));
  ...
  Future<void> _initEngine() async {
    ...
    engine.registerEventHandler(RtcEngineEventHandler(
      ...
      onJoinChannelSuccess: (RtcConnection connection, int elapsed) {
        ...
        context.read<VideoCallBloc>().add(IsJoinedChanged(true));
      },
      ...
      onUserJoined: (RtcConnection connection, int remoteUid, int elapsed) async {
        ...
        context.read<VideoCallBloc>().add(RemoteUidChanged(remoteUid));
        context.read<VideoCallBloc>().add(IsShowAvatarChanged(false));
        await player.pause();
        if (_callRole == "anchor") {
          _callTime();
          is_calltimer = true;
    ...
    await joinChannel();
    context.read<VideoCallBloc>().add(IsReadyPreviewChanged(true));
  ...
  _callTime() async {
    calltimer = Timer.periodic(Duration(seconds: 1), (timer) {
      ...
      if (call_h == 0) {
        context.read<VideoCallBloc>().add(CallTimeChanged("${m}:${s}"));
        _callTimeNum = "${call_m} m and ${call_s} s";
      } else {
        context.read<VideoCallBloc>().add(CallTimeChanged("${h}:${m}:${s}"));
  ...
  Future<String> _getToken() async {
    ...
    if (res.code == 0) {
      ...
      context.read<VideoCallBloc>().add(ChannelIdChanged(channel_name));
      return res.data!;
    }
    return "";
  }
  ...
  leaveChannel(bool is_self) async {
    EasyLoading.show(
        ...
        maskType: EasyLoadingMaskType.clear,
        dismissOnTap: true);
    context.read<VideoCallBloc>().add(IsJoinedChanged(false));
    context.read<VideoCallBloc>().add(SwitchCamerasChanged(true));
    context.read<VideoCallBloc>().add(RemoteUidChanged(0));
    context.read<VideoCallBloc>().add(IsShowAvatarChanged(true));
  ...
  switchCamera() async {
    ...
    var state_new = context.read<VideoCallBloc>().state;
    context.read<VideoCallBloc>().add(SwitchCamerasChanged(!state_new.switchCameras));
  }
