...
class ChatLogic {
  ...
  void _chatSnapshots(){
    ...
    mContext.read<ChatBloc>().add(const MsgContentClear());
    ...
    listener = messages.snapshots().listen(
        ...
        for (var element in tempMsgList.reversed) {
          mContext.read<ChatBloc>().add(MsgContentListChanged(element));
        }
    ...
    myscrollController.addListener((){
      ...
      if((myscrollController.offset+10)>myscrollController.position.maxScrollExtent){
        if(isloadmore){
          mContext.read<ChatBloc>().add(const isloadingChanged(true));
          isloadmore = false;
          _asyncLoadMoreData(mContext.read<ChatBloc>().state.msgcontentList.toList().length);
        }
  ...
  _asyncLoadMoreData(int page) async{
    var mContext = context;
    var state = mContext.read<ChatBloc>().state;
    ...
    if(messages.docs.isNotEmpty){
      messages.docs.forEach((element) {
        ...
        mContext.read<ChatBloc>().add(MsgContentAdd(data));
        print(data.content);
      });
    ...
    mContext.read<ChatBloc>().add(isloadingChanged(false));
  }
  ...
  sendImageMessage(String url) async{
    context.read<ChatBloc>().add(moreStatusChanged(false));
    print("---------------chat-----------------");
    final content = Msgcontent(
      token: userProfile.token,
      content: url,
  ...
  goMore(){
    var more_status = context.read<ChatBloc>().state.more_status;
    context.read<ChatBloc>().add(moreStatusChanged(more_status?false:true));
  }

  photoImg(Msgcontent item){
    Navigator.of(context).pushNamed(AppRoutes.Photoview,arguments: {"url": item.content});
  }
  ...
  goVoiceCall() async{
    ...
    if(microphoneStatus){
    ChatState state = context.read<ChatBloc>().state;
    Navigator.of(context).pushNamed(AppRoutes.VoiceCall, arguments: {
  ...
  goVideoCall() async{
    ...
    ChatState state = context.read<ChatBloc>().state;
    Navigator.of(context).pushNamed(AppRoutes.VideoCall, arguments: {
      "to_token": state.to_token,
      "to_name": state.to_name,
      "to_avatar": state.to_avatar,
      "doc_id": doc_id,
